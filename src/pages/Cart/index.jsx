...
export default function Cart() {
  ...
  const articleCommands =
    cart.articleCommands?.length > 0 ? cart.articleCommands : null;
  const totalCost = cart?.articleCommands?.reduce(
    (count, articleCommand) =>
      count + articleCommand.quantity * articleCommand.stock.article.price,
    0
  );

  return (
    <>
      <Section id="cart" title="Mon panier">
          ...
          <div className="flex">
            <div id="cartContent">
              <div className="flex col">
                {articleCommands &&
                  articleCommands.map((articleCommand, index) => {
                    return (
                      <CartArticleCommand
                        key={index}
